  ...
  <a-drawer
    ...
    <a-form :form="form" layout="vertical">
      <a-row :gutter="10" v-if="purchaseData != null">
        ...
        <a-col :span="12">
          <a-form-item label='产品答疑标题' v-bind="formItemLayout">
            <a-input disabled v-model="purchaseData.title"/>
          </a-form-item>
        </a-col>
        <a-col :span="24">
          <a-form-item label='问题情况' v-bind="formItemLayout">
            <a-textarea :rows="6" disabled v-model="purchaseData.cause"/>
          </a-form-item>
        </a-col>
        <a-col :span="8">
          <a-form-item label='采购销售店'>
            <a-select @change="pharmacyCheck" v-decorator="[
              'pharmacyId',
              { rules: [{ required: true, message: '请输入采购销售店!' }] }
              ]">
      ...
      <a-row :gutter="10">
        <a-divider orientation="left">
          <span style="font-size: 13px">农产品信息</span>
        </a-divider>
        <a-col :span="24">
          <a-table :columns="columns" :data-source="dataList" :pagination="false">
            ...
            <template slot="typeIdShow" slot-scope="text, record">
              <span v-if="record.classification == 1">粮食</span>
              <span v-if="record.classification == 2">蔬菜</span>
              <span v-if="record.classification == 3">水果</span>
              <span v-if="record.classification == 4">养殖</span>
              <span v-if="record.classification == 5">其他</span>
            </template>
